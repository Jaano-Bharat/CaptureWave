...
data class VideoFormat(
    val name: String, val codec: Int, val extension: String, val format: Int
) {
    companion object {
        val codecs = mutableListOf(
            VideoFormat(
                "H.264", MediaRecorder.VideoEncoder.H264, "mp4", MediaRecorder.OutputFormat.MPEG_4
            ), VideoFormat(
                "webm (VP8)",
        ...
        fun getCurrent() = codecs.first {
            it.codec == Preferences.prefs.getInt(
                Preferences.videoCodecKey, MediaRecorder.VideoEncoder.H264
            )
        }
    }
}