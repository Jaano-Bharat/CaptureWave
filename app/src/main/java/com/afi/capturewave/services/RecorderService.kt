...
import com.afi.capturewave.receivers.NotificationReceiver
import com.afi.capturewave.ui.MainActivity
import com.afi.capturewave.util.NotificationHelper
import com.afi.capturewave.util.PermissionHelper
...
abstract class RecorderService : Service() {
    ...
    override fun onCreate() {
        ...
        if (fgServiceType != null && Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            startForeground(
                NotificationHelper.RECORDING_NOTIFICATION_ID, notification.build(), fgServiceType!!
            )
        } else {
        ...
        registerReceiver(
            bluetoothReceiver, IntentFilter(AudioManager.ACTION_SCO_AUDIO_STATE_UPDATED)
        )
    ...
    private fun buildNotification(): NotificationCompat.Builder {
        ...
        val stopAction = NotificationCompat.Action.Builder(
            null, getString(R.string.stop), getPendingIntent(stopIntent, 2)
        )

        val resumeOrPauseIntent = Intent(RECORDER_INTENT_ACTION).putExtra(
            ACTION_EXTRA_KEY, PAUSE_RESUME_ACTION
        )
        val resumeOrPauseAction = NotificationCompat.Action.Builder(
            null, if (recorderState == RecorderState.ACTIVE) {
        ...
        return NotificationCompat.Builder(
            this, NotificationHelper.RECORDING_NOTIFICATION_CHANNEL
        ).setContentTitle(notificationTitle).setSmallIcon(R.drawable.ic_notification)
            .setPriority(NotificationCompat.PRIORITY_LOW).setSilent(true)
            .setOngoing(recorderState == RecorderState.ACTIVE).addAction(stopAction.build()).apply {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                    addAction(
                        resumeOrPauseAction.build()
                    )
                }
            }.setUsesChronometer(true).setContentIntent(getActivityIntent())
    }
    ...
    fun updateNotification() {
        ...
        NotificationManagerCompat.from(this).notify(
            NotificationHelper.RECORDING_NOTIFICATION_ID, notification
        )
    }
    ...
    private fun createRecordingFinishedNotification() {
        ...
        val deleteAction = NotificationCompat.Action.Builder(
            null, getString(R.string.delete), getPendingIntent(
                Intent(this, NotificationReceiver::class.java).putExtra(
                    FILE_NAME_EXTRA_KEY, outputFile?.name.toString()
                ).putExtra(ACTION_EXTRA_KEY, DELETE_ACTION), 4
            )
        )
        val shareAction = NotificationCompat.Action.Builder(
            null, getString(R.string.share), getPendingIntent(
                Intent(this, NotificationReceiver::class.java).putExtra(
                    FILE_NAME_EXTRA_KEY, outputFile?.name.toString()
                ).putExtra(ACTION_EXTRA_KEY, SHARE_ACTION), 5
            )
        )

        val notification = NotificationCompat.Builder(
            this, NotificationHelper.RECORDING_FINISHED_N_CHANNEL
        ).setContentTitle(getString(R.string.recording_finished)).setContentText(outputFile?.name)
            .setSmallIcon(R.drawable.ic_notification).addAction(deleteAction.build())
            // / .addAction(shareAction.build())
            .setContentIntent(getActivityIntent()).setAutoCancel(true)
    ...
    private fun getActivityIntent(): PendingIntent {
        return PendingIntent.getActivity(
            this, 6, Intent(this, MainActivity::class.java), PendingIntent.FLAG_IMMUTABLE
        )
    }
    ...
    companion object {
        const val RECORDER_INTENT_ACTION = "com.afi.capturewave.RECORDER_ACTION"
