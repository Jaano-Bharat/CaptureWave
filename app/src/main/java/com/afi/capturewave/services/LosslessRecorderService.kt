...
class LosslessRecorderService : RecorderService() {
    ...
    override fun start() {
        ...
        val audioFormat: AudioFormat =
            AudioFormat.Builder().setSampleRate(SAMPLING_RATE).setChannelMask(CHANNEL_IN)
        ...
        pcmConverter = PcmConverter(
            audioFormat.sampleRate.toLong(), audioFormat.channelCount, 2 * 8
        )

        audioRecorder?.startRecording()

        outputFile = DocumentFile.fromFile(File(filesDir, "temp.pcm").also {
            it.createNewFile()
        })
    ...
    private fun convertToWav() {
        ...
        val outputStream =
            (application as App).fileRepository.getOutputFile(FILE_NAME_EXTENSION_WAV)?.let {
    ...
    companion object {
        ...
        private val BUFFER_SIZE_IN_BYTES = 2 * AudioRecord.getMinBufferSize(
            SAMPLING_RATE, CHANNEL_IN, FORMAT
        )
    }
}