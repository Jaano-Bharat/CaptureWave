...
class MediaTrimmer {
    ...
    suspend fun trimMedia(
        context: Context, inputFile: DocumentFile, startMs: Long, endMs: Long
    ): Boolean {
        ...
        return withContext(Dispatchers.IO) {
            ...
            val outputFile = (context.applicationContext as App).fileRepository.getOutputFile(
                extension = extension ?: "mp4", prefix = "Trim_"
            )
    ...
    private suspend fun trimMediaFile(
        inputPfd: ParcelFileDescriptor, outputPfd: ParcelFileDescriptor, startMs: Long, endMs: Long
    ): Boolean {
        return withContext(Dispatchers.IO) {
                    ...
                    if (bufferInfo.size < 0) {
                        ...
                        if (bufferInfo.presentationTimeUs > endMs * 1000) {
                            ...
                            muxer.writeSampleData(
                                indexMap.get(trackIndex), destinationBuffer, bufferInfo
                            )
                            extractor.advance()
                        }
                    }
